...
{   
    ...
    public function index() {
        $this->_model = $this->loadModel("mensajecancelado");
        ...
        $this->_view->datos = $this->_model->dql("SELECT m FROM Entities\MensajeCancelado m JOIN m.mensaje men WHERE men.estadomensaje =:estadomensaje AND men.usuario =:usuario",array('estadomensaje' => 4, 'usuario' => Session::get('codigo')));
    ...
    private function obj($new = true) {
        ...
        $this->_model->getInstance()->setHoras($this->getInt('horas'));
        if($new){
            $this->_model->save(); 
